...
本文面向第一次接触本项目的开发或测试人员。目标是先说明任务，再给出由浅入深、可以实际执行的复现路径。会议中汇报的主体是前两轮实验；第三轮并行实验作为后续补充，也已纳入本文。
...
- 优化 NGSPICE 源码以 Git submodule 固定在提交 `7a76e17d46ba49aa74f417151f8b8311c488760a`，其官方基线为 ngspice `pre-master-47` 提交 `eb68de42d0ca8c97efd92f8d7528e7e7841f5fc9`。
...
因此，需要补到仓库的是可公开源码、配置模板和复现说明；不应上传个人 `.benchmark-env`、HSPICE 安装包、license 文件或服务器私有日志。
...
`git submodule status` 应显示优化源码提交 `7a76e17...`。公司若要修改优化逻辑，应在 `upstream/ngspice_for_sizing/` 对应的源码仓库中建立自己的分支，并同步更新本仓库的 submodule 指针；不要只复制一个本地 build 目录。
...
1. 源码 commit 或 patch，以及修改动机和适用边界。
2. `environment.txt`、源码 commit、binary SHA-256 和编译命令。
3. smoke gate 与正式 matrix gate。
4. 原始 JSON、汇总 CSV、主要曲线和与基线的加速比。
5. 同模拟器正确性误差、失败样本和未覆盖范围。
6. 一页结论：哪些场景加速、哪些场景无收益、瓶颈在 setup/load/LU/启动/许可证中的哪一项。

公开提交前必须运行 release audit，并确认不包含 HSPICE binary、license、守护进程日志、凭据、个人环境文件或公司私有模型。
